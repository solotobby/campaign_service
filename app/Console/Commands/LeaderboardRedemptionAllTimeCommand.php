...
class LeaderboardRedemptionAllTimeCommand extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'leaderboard:alltimeredemption';
    ...
    {
        $campaigns = Campaign::whereDate('end_date', (Carbon::now())->toDateString())->get();
        ...
        foreach ($campaigns as $campaign) {
            ...
            if (count($leaderboard) > 0) {
                $audiences = (new GetBatchAudience(['user_ids' => $leaderboard->pluck('audience_id')]))->run();
            ...
            foreach ($leaderboard as $key => $player) {
                if (array_key_exists($player->audience_id, $audiences)) {
                    ...
                    // get reward
                    $reward = $campaign->leaderboardRewards->where('frequency', 'ALL-TIME')
